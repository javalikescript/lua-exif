...
static int luaexif_data_load(lua_State *l) {
	...
	exif_data_load_data(data->exifData, buffer, length);
	return 0;
}
		...
		if (lua_istable(l, -1)) {
			...
			while (lua_next(l, -2)) {
				if (lua_isstring(l, -2) && lua_istable(l, -1)) {
					...
					unsigned long entryComponents = 0;
					//unsigned char *data;
					unsigned int entrySize = 0;
					const char *entryValue = "";
					int entryDataLength = -1;
					const char *entryName = lua_tostring(l, -2);
					trace("entry \"%s\"\n", entryName);
					...
					if ((entryTag == 0) || (entryFormat == 0) || ((entryDataLength >= 0) && (entryComponents != entryDataLength))) {
					...
					if (entryFormat == EXIF_FORMAT_ASCII) {
						...
						for (i = 1; i <= entryDataLength; i++) {
							...
							case EXIF_FORMAT_BYTE:
								entry->data[offset] = (ExifByte) d;
								break;
							case EXIF_FORMAT_SBYTE:
								entry->data[offset] = (ExifSByte) d;
								break;
							case EXIF_FORMAT_SHORT:
								exif_set_short(entry->data + offset, bo, (ExifShort) d);
								break;
							case EXIF_FORMAT_SSHORT:
								exif_set_sshort(entry->data + offset, bo, (ExifSShort) d);
								break;
							case EXIF_FORMAT_LONG:
								exif_set_long(entry->data + offset, bo, (ExifLong) d);
								break;
							case EXIF_FORMAT_SLONG:
								exif_set_slong(entry->data + offset, bo, (ExifSLong) d);
								break;
							case EXIF_FORMAT_DOUBLE:
								//exif_set_do(entry->data + offset, bo);
								break;
							case EXIF_FORMAT_RATIONAL:
								//exif_set_rational(entry->data + offset, bo);
								break;
							case EXIF_FORMAT_SRATIONAL:
								//exif_set_srational(entry->data + offset, bo);
								break;
    		...
    		if (*value) {
            	...
            	if (entry->format != EXIF_FORMAT_ASCII) {
					...
					for (i = 1; i <= entry->components; i++) {
						...
						switch (entry->format) {
						case EXIF_FORMAT_BYTE:
							d = (ExifByte) entry->data[offset];
							break;
						case EXIF_FORMAT_SBYTE:
							d = (ExifSByte) entry->data[offset];
							break;
						case EXIF_FORMAT_SHORT:
							d = exif_get_short(entry->data + offset, bo);
							break;
						case EXIF_FORMAT_SSHORT:
							d = exif_get_sshort(entry->data + offset, bo);
							break;
						case EXIF_FORMAT_LONG:
							d = exif_get_long(entry->data + offset, bo);
							break;
						case EXIF_FORMAT_SLONG:
							d = exif_get_slong(entry->data + offset, bo);
							break;
						case EXIF_FORMAT_DOUBLE:
							//d = exif_get_do(entry->data + offset, bo);
							break;
						case EXIF_FORMAT_RATIONAL:
							//exif_get_rational(entry->data + offset, bo);
							break;
						case EXIF_FORMAT_SRATIONAL:
							//exif_get_srational(entry->data + offset, bo);
							break;
						case EXIF_FORMAT_FLOAT:
						case EXIF_FORMAT_UNDEFINED:
							break;
						...
						trace("data[%d] = %f\n", i, d);
						lua_pushnumber(l, d);
						lua_rawseti(l, -2, i); // add value in data table list
						offset += fs;
					}
